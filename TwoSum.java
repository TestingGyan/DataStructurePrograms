import java.util.HashMap;

public class TwoSum {


    /*
     * Brute Force
     * 
     * TC: O(n^2)
     * SC: O(1)
     * 
     * 
     */
    public static int[] twoSumB(int [] arr,int target){

        int n=arr.length;
        int []res=new int[2];
        for(int i=0;i<n;i++){ //loop to pick one array element at a time

            for(int j=0;j<n;j++){  //loop to check with elements next to picked array element

                if((arr[i]+arr[j])==target){ //comparing sum to target
                    res[0]=i;
                    res[1]=j;
                    return res;
                }
            }
        }
        return res;

    }

    






    
    /*
     * Efficient
     * 
     * TC: O(n)
     * SC: O(n)
     * 
     * 
     */
    public static int[] twoSumE(int[]arr,int target){

        int n=arr.length;
        HashMap<Integer,Integer> map=new HashMap<>();
        int [] res=new int[2];

        for(int i=0;i<n;i++){
            if(map.containsKey(target-arr[i])){ //checking if map contains target-arr[i]

                res[0]=i;
                res[1]=map.get(target-arr[i]);
                return res;
            }
            map.put(arr[i], i); //Inserting the element in map if target-arr[i] not found
        }
        return res;
    }




    public static void main(String[] args) {


        int [] arr={1,2,3,4};
 
        int [] res=twoSumB(arr, 9);
        System.out.println("("+res[0]+","+res[1]+")");

        int [] res1=twoSumE(arr, 9);
        System.out.println("("+res1[0]+","+res1[1]+")");

        
    }
    
}
